...
export class TerisRule {
   ...
   * @returns 为true就可以移动
   */
  static canIMove(teris: SquareGroup, targetCenterPoint: Point): boolean {
    //得到移动后各个小块的位置
    const targetShape: Point[] = teris.shape.map((it) => {
      return {
  ...
  static move(
    teris: SquareGroup,direction: MoveDirection ): boolean;
  ...
  static move(
    ...
    if (isPoint(targetCenterPointOrDirection)) {
      if (this.canIMove(teris, targetCenterPointOrDirection)) {
